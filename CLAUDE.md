...
- `index.html` — the page shell (~140 lines): meta tags, Leaflet CSS/JS from unpkg, then `style.css`, `route.js`, `app.js`. Must stay named `index.html` — GitHub Pages serves it at the site root.
- `style.css` — all styling (~445 lines).
...
- `app.js` — the application logic (~910 lines), wrapped in `(function(){ ... })();`.
- `manifest.webmanifest`, `sw.js`, `icons/` — the PWA layer (see below).
...
The app is installable on Android/iOS ("Ajouter à l'écran d'accueil" / "Installer l'application") and works without network once it has been opened online.

- `manifest.webmanifest` — `display: standalone`, relative `start_url`/`scope` (`.`), so it keeps working under the `/geants-300/` sub-path. Referenced from `index.html` along with the Apple meta tags.
- `icons/` — PNGs generated by a throwaway pure-stdlib Python script (no PIL/ImageMagick on this machine); there is no icon source file in the repo. Re-create them by hand if the design should change.
- `sw.js` — registered at the end of `index.html`. Three caches: `shell` (app files, precached, stale-while-revalidate), `vendor` (Leaflet from unpkg, precache failures tolerated), `tiles` (OSM tiles seen while browsing, cache-first, capped at 1200 entries). Navigations are network-first with a cached `index.html` fallback.
- **Bump `VERSION` in `sw.js` when shipping a change** that must reach installed users immediately; otherwise assets refresh one reload late.
- Only tiles the user has actually panned over are cached — to have the route offline in the mountains, browse the whole track once while online.
...
The app derives everything from `ROUTE.track` at load time — there is no persistence beyond in-memory state.
...
4. **Rendering**: an SVG elevation profile (with gradient-colored segments matching the legend: flat/moderate climb/steep climb/descent), a schematic route/map panel, a splits table, and header stat tiles (distance, D+, start/finish/total time) — all built via DOM/SVG element construction in JS, no templating library.
